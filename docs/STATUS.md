...
| image | 예 | 부분 | 예 | 부분 (JSON bytes 포함, HTML/Markdown asset 파일, TXT/SVG 대체 텍스트) | 이미지 테두리(색·굵기, 선종류는 solid 가정), 회색조·임계 흑백·Pattern8x8 효과 구분, 회전·가로/세로 반전, 내부 padding, 캡션 배치, crop 좌표, 밝기·대비 원시 값, 글자처럼 취급·wrap·가로/세로 기준·정렬·오프셋은 IR에 매핑됨. 원본 크기가 명확한 HWPX crop과 HWPX 이미지 opacity는 HTML에 적용한다. 임계 흑백은 회색조로 근사하고 Pattern8x8·밝기·대비는 warning과 원본 바이트를 유지한다. 페이지 좌표 기반 본문 배치는 semantic exporter에서 선형화한다. anchor는 아직 미지원이다. HWPX 폴백은 이미지 참조/설명/테두리/배치/crop/`inMargin` 관련 속성 alias 일부를 복구한다. bin data 없으면 `UnknownBlock`으로 남기며 alt/description 계열 속성은 fallback text로 보존한다. `Resource::Binary`는 asset으로 안 씀. |
