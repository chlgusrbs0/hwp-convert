...
| table | 예 | 예 | 예 | 부분 (JSON/HTML 구조 유지, 헤더셀은 `<th>`, 셀 수직정렬 CSS, TXT/SVG 평문, Markdown 단순 표만) | 셀 `is_header`, 수직정렬, 폭/높이/padding, 4면 테두리, 표 전체 너비·높이/바깥 여백과 행 높이는 매핑됨. HWPX 폴백은 borderFill, 셀 margin, 표 `sz/outMargin` 및 행 높이 속성 alias 일부를 복구한다. cell spacing, page break, borderFill zone은 아직 경고 후 생략한다. 테두리 굵기와 wave/3D 선종류는 근사이므로 실문서 fixture 검증이 필요하다. |
...
| shape | 예 | 부분 | 부분 | 부분 (모두 `[shape: ...]` placeholder) | `kind`, `fallback_text`, `description`과 HWP 도형의 기본 너비·높이·X/Y 오프셋을 보존한다. border/fill/text box/caption/child shape은 제한적이다. |
