...
| resource | 예 | 부분 | 부분 | 부분 (JSON store 보존, HTML/Markdown `Resource::Image`만 파일로) | HWP에서 이미지가 참조한 BinData는 `ImageResource`, 그 밖의 로드된 임베디드 BinData는 `BinaryResource`로 보존한다. 외부 링크형 BinData 경로와 HWPX 비이미지 manifest resource, binary asset 파일 출력은 아직 제한적이다. |
