...
| table | 예 | 예 | 예 | 부분 (JSON/HTML 구조 유지, 반복 머리글은 `<thead>`, 헤더셀은 `<th>`, 셀 수직정렬 CSS, TXT/SVG 평문, Markdown 단순 표만) | 셀 `is_header`, 수직정렬, 폭/높이/padding, 4면 테두리, 표 전체 너비·높이/바깥 여백, 행 높이, HWP 반복 머리글과 페이지 분할 규칙은 매핑됨. HWPX 폴백은 borderFill, 셀 margin, 표 `sz/outMargin` 및 행 높이 속성 alias 일부를 복구한다. cell spacing, HWPX page break, borderFill zone은 아직 경고 후 생략한다. 테두리 굵기와 wave/3D 선종류는 근사이므로 실문서 fixture 검증이 필요하다. |
...
| image | 예 | 부분 | 예 | 부분 (JSON bytes 포함, HTML/Markdown asset 파일, TXT/SVG 대체 텍스트) | 이미지 테두리(색·굵기, 선종류는 solid 가정), 흑백(grayscale), 회전과 가로·세로 반전은 매핑됨. crop, 밝기/대비, opacity, 내부 padding, wrap, anchor, 배치는 아직(미지원은 warning). HWPX 폴백은 이미지 참조/설명/테두리/배치/crop/inner margin 관련 속성 alias 일부를 복구한다. bin data 없으면 `UnknownBlock`으로 남기며 alt/description 계열 속성은 fallback text로 보존한다. `Resource::Binary`는 asset으로 안 씀. |
...
| shape | 예 | 부분 | 부분 | 부분 (모두 `[shape: ...]` placeholder) | `kind`, `fallback_text`, `description`과 HWP 도형의 기본 너비·높이·X/Y 오프셋, 회전·반전, 표준 테두리와 패턴 없는 단색 채우기, 텍스트 상자 안쪽 여백·세로 정렬을 보존한다. HWPX도 회전·반전을 복구한다. HWPX 도형 텍스트 상자 속성은 아직 미매핑이다. pattern/image/gradient fill, shadow, caption/child shape은 제한적이다. |
