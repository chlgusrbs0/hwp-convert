...
| section/page layout | 예 | 부분 | 예 | JSON 보존 | HWP 구역의 용지 크기·방향·제본, 모든 여백, 번호 시작값, 기본 탭/단 간격, 숨김 플래그, 각주·미주 모양, 페이지 테두리 참조와 불투명 확장 레코드를 보존한다. master page와 문단 중간의 다단 전환은 아직 구조화하지 않는다. HWPX 구역 레이아웃 복구는 미구현이다. |
