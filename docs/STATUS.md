...
| equation | 예 | 부분 | 예 | 부분 (JSON 보존, HTML 표시 스타일, 나머지 `[equation: ...]`, Markdown은 `Latex`일 때만 `$$`) | bridge가 `EquationKind::PlainText`를 생성하며 rHWP의 글꼴·크기·색·기준선·크기·오프셋·버전, 공통 개체 인스턴스 ID·배치·크기 기준·원본 control data를 보존한다. 상대 크기는 px로 오인하지 않는다. LaTeX/MathML 판별, numbering, resource 연결 없음. |
...
같은 source layout에는 rHWP 공개 원본 문자 수·control mask·PARA_TEXT 존재 여부·UTF-16 문자 위치·글자모양 참조·range tag와 PARA_HEADER/CTRL_DATA 확장 바이트도 함께 보존한다. 불투명 바이트는 Base64이며 의미를 독자 해석하지 않는다.
...
HWP 도형·이미지의 rHWP 공개 구성요소 control ID, ID 반복 여부, 그룹 단계, 로컬 파일 버전, 원본 flip 비트와 불투명 렌더링 바이트도 공용 `ShapeTransform`에 보존한다. 정수 source metadata는 HTML `data-*`로 출력하고 렌더링 바이트는 JSON에 Base64로만 남긴다.
...
이미지 효과는 원본·회색조·흑백·Pattern8x8을 IR에서 구분한다. HTML은 회색조를 그대로 적용하고 흑백은 회색조와 고대비로 근사하며, Pattern8x8은 rHWP SVG와 같은 회색조 fallback을 사용한다.
...
HWP 도형·이미지는 rHWP 공개 공통 개체 인스턴스 ID와 component 인스턴스 ID를 보존한다. 도형 component ID와 공통 ID는 rHWP 연결선의 `SubjectID` 대상 조회에 사용되므로, 연결선 참조와 대상 개체의 관계를 IR과 HTML `data-*`에서 추적할 수 있다.

HWP 표·도형·이미지·수식의 폭·높이 기준과 원본 수치를 `ObjectPlacement`에 보존한다. 절대 기준은 px 표시 크기로 변환하고, 종이·쪽·단·문단 기준의 1/10000 비율 값은 절대 px로 오해하지 않도록 표시 크기에서 제외하며 HTML `data-*`와 warning으로 남긴다.
...
HWP 표·도형·이미지·수식의 공통 개체 control 뒤에 남은 rHWP 공개 확장 바이트는 `ObjectPlacement.raw_control_extension`에 Base64로 보존한다. 해석되지 않은 바이트의 의미를 추정하지 않으며 JSON 외 semantic exporter는 이를 시각화하지 않는다.
