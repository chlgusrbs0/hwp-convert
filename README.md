...
- HTML/Markdown exporter는 이미지 resource bytes를 출력 파일 stem 기준의 `<stem>_assets/images/` 디렉터리에 저장하고 `<stem>_assets/images/...` 상대 경로로 참조한다. 예: `out/sample.html`과 `out/sample.md`는 `out/sample_assets/images/image-1.png`를 쓰고 문서에서는 `sample_assets/images/image-1.png`로 참조한다. 기존 공용 sibling `images/` 경로는 여러 문서의 `image-1.png` 충돌을 피하기 위해 사용하지 않는다.
- 현재 CLI의 `--to svg`는 semantic/plain-text 기반 SVG exporter이며 RenderSnapshot visual SVG 경로와 분리되어 있다.
